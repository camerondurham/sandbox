...
public:
  ...
  SharedPtr()
      : mData(nullptr), mBlock(nullptr)
  {
  }

  // Assume we are the first shared ptr to data
  SharedPtr(T *data)
      : mData(data), mBlock(new ControlBlock)
  {
  ...
  SharedPtr(const SharedPtr<T> &other)
      : mData(other.mData), mBlock(other.mBlock)
  {
  ...
  SharedPtr(SharedPtr<T> &&other) noexcept
      : mData(std::move(other.mData)), mBlock(std::move(other.mBlock))
  {
    other.mData = nullptr;
...
public:
  ...
  WeakPtr(const SharedPtr<T> &shared)
      : mData(shared.mData), mBlock(shared.mBlock)
  {
    mBlock->mWeakRefs++;
  }
