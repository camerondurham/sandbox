...
// Generates a set of unique random integers of the given length, with the given
// seed.
std::set<int> makeRandomIntSet(size_t count, RandomSeed seed) {
  ...
  std::uniform_int_distribution<int> distributor(-5 * ((int)count),
                                                 5 * ((int)count));

  // keep adding numbers until we have enough.
  // We can't use a for-loop because some numbers are duplicates and won't
  // count.
...
std::vector<int> makeRandomIntVector(size_t count, RandomSeed seed,
                                     bool allowDuplicates) {
  return makeRandomNumberVector<int>(count, -5 * static_cast<int>(count),
                                     5 * static_cast<int>(count), seed,
                                     allowDuplicates);
}

std::vector<RandomSeed> makeRandomSeedVector(size_t count, RandomSeed seed) {
  return makeRandomNumberVector<RandomSeed>(
      count, 0, 10 * static_cast<RandomSeed>(count), seed, false);
}

std::vector<std::string> makeRandomNumStringVector(size_t count,
                                                   RandomSeed seed) {
  // set up random number generator
  std::mt19937 randEngine;
  randEngine.seed(seed);
...
std::vector<std::string> makeRandomAlphaStringVector(size_t count,
                                                     RandomSeed seed,
                                                     size_t stringLength,
                                                     bool allowDuplicates) {
  // set up random number generator
